...
impl Activity {
	...
	pub fn create(db: &mut MysqlConnection, item: &CreateActivity) -> QueryResult<Self> {
		use crate::schema::activity::dsl::*;

		trace!("Inserting into activity table: {:?}", item);
		db.transaction(|conn| {
			insert_into(activity).values(item).execute(conn)?;
			activity
	...
	pub fn read(db: &mut MysqlConnection, param_id: i32) -> QueryResult<Self> {
		...
		activity.filter(id.eq(param_id)).first::<Self>(db)
	}
	...
	pub fn paginate(
		...
		let total_items = activity.count().get_result(db)?;
		let items = activity
			.limit(page_size)
	...
	pub fn update(
		...
		db.transaction(|conn| {
			diesel::update(activity.filter(id.eq(param_id)))
				.set(item)
				.execute(conn)?;
			activity.filter(id.eq(param_id)).first::<Self>(conn)
		})
	}
	...
	pub fn delete(db: &mut MysqlConnection, param_id: i32) -> QueryResult<usize> {
		...
		diesel::delete(activity.filter(id.eq(param_id))).execute(db)
	}
	...
	pub fn last_page(db: &mut MysqlConnection, page_size: i64) -> QueryResult<i64> {
		...
		let total_items: i64 = activity.count().get_result(db)?;
		// index starts at 0
